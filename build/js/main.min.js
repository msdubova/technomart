...
function addClasses() {
  ...
  const suffixes = prepareClasses(['initialized', params.direction, {
    ...
    'watch-progress': params.watchrange__slidesProgress
...
/* harmony import */ var _range__slide_index_js__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ./range__slide/index.js */ "../node_modules/swiper/core/range__slide/index.js");
...
const prototypes = {
  ...
  range__slide: _range__slide_index_js__WEBPACK_IMPORTED_MODULE_11__["default"],
...
class Swiper {
  ...
  recalcrange__slides() {
    ...
    swiper.range__slides = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementChildren"])(range__slidesEl, `.${params.range__slideClass}, swiper-range__slide`);
  }
  enable() {
    const swiper = this;
  ...
  emitrange__slidesClasses() {
    ...
    if (!swiper.params._emitClasses || !swiper.el) return;
    const updates = [];
    swiper.range__slides.forEach(range__slideEl => {
  ...
  mount(element) {
    ...
    if (!wrapperEl && swiper.params.createElements) {
      ...
      el.append(wrapperEl);
      Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementChildren"])(el, `.${swiper.params.range__slideClass}`).forEach(range__slideEl => {
    ...
    Object.assign(swiper, {
      el,
      wrapperEl,
      range__slidesEl: swiper.isElement ? el : wrapperEl,
      mounted: true,
      // RTL
  ...
  init(el) {
    ...
    if (swiper.params.loop && swiper.virtual && swiper.params.virtual.enabled) {
      swiper.range__slideTo(swiper.params.initialrange__slide + swiper.virtual.range__slidesBefore, 0, swiper.params.runCallbacksOnInit, false, true);
    } else {
  ...
  destroy(deleteInstance = true, cleanStyles = true) {
    ...
    if (cleanStyles) {
      ...
      if (range__slides && range__slides.length) {
        range__slides.forEach(range__slideEl => {
          range__slideEl.classList.remove(params.range__slideVisibleClass, params.range__slideActiveClass, params.range__slideNextClass, params.range__slidePrevClass);
          range__slideEl.removeAttribute('style');
          range__slideEl.removeAttribute('data-swiper-range__slide-index');
        });
      }
    }
    swiper.emit('destroy');
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  maxBackfaceHiddenrange__slides: 10,
  // NS
...
function onResize() {
  ...
  if ((params.range__slidesPerView === 'auto' || params.range__slidesPerView > 1) && swiper.isEnd && !swiper.isBeginning && !swiper.params.centeredrange__slides && !isVirtualLoop) {
    swiper.range__slideTo(swiper.range__slides.length - 1, 0, false, true);
  } else {
    if (swiper.params.loop && !isVirtual) {
      swiper.range__slideToLoop(swiper.realIndex, 0, false, true);
    } else {
...
function onTouchEnd(event) {
  ...
  if (params.rewind) {
    if (swiper.isBeginning) {
      rewindLastIndex = swiper.params.virtual && swiper.params.virtual.enabled && swiper.virtual ? swiper.virtual.range__slides.length - 1 : swiper.range__slides.length - 1;
  ...
  if (timeDiff > params.longSwipesMs) {
    ...
    if (swiper.swipeDirection === 'next') {
      if (ratio >= params.longSwipesRatio) swiper.range__slideTo(params.rewind && swiper.isEnd ? rewindFirstIndex : stopIndex + increment);else swiper.range__slideTo(stopIndex);
    }
    if (swiper.swipeDirection === 'prev') {
      if (ratio > 1 - params.longSwipesRatio) {
        swiper.range__slideTo(stopIndex + increment);
      } else if (rewindLastIndex !== null && ratio < 0 && Math.abs(ratio) > params.longSwipesRatio) {
        swiper.range__slideTo(rewindLastIndex);
      } else {
    ...
    if (!isNavButtonTarget) {
      if (swiper.swipeDirection === 'next') {
        swiper.range__slideTo(rewindFirstIndex !== null ? rewindFirstIndex : stopIndex + increment);
      }
      if (swiper.swipeDirection === 'prev') {
        swiper.range__slideTo(rewindLastIndex !== null ? rewindLastIndex : stopIndex);
      }
...
function onTouchMove(event) {
  ...
  if (diff > 0) {
    if (isLoop && !loopFixed && data.currentTranslate > (params.centeredrange__slides ? swiper.minTranslate() - swiper.size / 2 : swiper.minTranslate())) {
      swiper.loopFix({
        direction: 'prev',
        setTranslate: true,
        activerange__slideIndex: 0
      });
    }
    ...
    if (isLoop && !loopFixed && data.currentTranslate < (params.centeredrange__slides ? swiper.maxTranslate() + swiper.size / 2 : swiper.maxTranslate())) {
      swiper.loopFix({
        direction: 'next',
        setTranslate: true,
        activerange__slideIndex: swiper.range__slides.length - (params.range__slidesPerView === 'auto' ? swiper.range__slidesPerViewDynamic() : Math.ceil(parseFloat(params.range__slidesPerView, 10)))
      });
    }
...
function loopCreate(range__slideRealIndex) {
  const swiper = this;
  const {
    params,
    range__slidesEl
  ...
  const range__slides = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementChildren"])(range__slidesEl, `.${params.range__slideClass}, swiper-range__slide`);
  range__slides.forEach((el, index) => {
    el.setAttribute('data-swiper-range__slide-index', index);
  });
  swiper.loopFix({
    range__slideRealIndex,
    direction: params.centeredrange__slides ? undefined : 'next'
  });
}
...
function loopDestroy() {
  ...
  if (!params.loop || swiper.virtual && swiper.params.virtual.enabled) return;
  swiper.recalcrange__slides();
  const newrange__slidesOrder = [];
  range__slides.forEach(range__slideEl => {
    const index = typeof range__slideEl.swiperrange__slideIndex === 'undefined' ? range__slideEl.getAttribute('data-swiper-range__slide-index') * 1 : range__slideEl.swiperrange__slideIndex;
    newrange__slidesOrder[index] = range__slideEl;
  });
  range__slides.forEach(range__slideEl => {
    range__slideEl.removeAttribute('data-swiper-range__slide-index');
  });
  newrange__slidesOrder.forEach(range__slideEl => {
    range__slidesEl.append(range__slideEl);
  });
  swiper.recalcrange__slides();
  swiper.range__slideTo(swiper.realIndex, 0);
}
...
function loopFix({
  ...
  if (swiper.virtual && params.virtual.enabled) {
    if (range__slideTo) {
      if (!params.centeredrange__slides && swiper.snapIndex === 0) {
        swiper.range__slideTo(swiper.virtual.range__slides.length, 0, false, true);
      } else if (params.centeredrange__slides && swiper.snapIndex < params.range__slidesPerView) {
        swiper.range__slideTo(swiper.virtual.range__slides.length + swiper.snapIndex, 0, false, true);
      } else if (swiper.snapIndex === swiper.snapGrid.length - 1) {
        swiper.range__slideTo(swiper.virtual.range__slidesBefore, 0, false, true);
      }
    }
    swiper.allowrange__slidePrev = allowrange__slidePrev;
    swiper.allowrange__slideNext = allowrange__slideNext;
  ...
  const range__slidesPerView = params.range__slidesPerView === 'auto' ? swiper.range__slidesPerViewDynamic() : Math.ceil(parseFloat(params.range__slidesPerView, 10));
  let loopedrange__slides = params.loopedrange__slides || range__slidesPerView;
  if (loopedrange__slides % params.range__slidesPerGroup !== 0) {
    loopedrange__slides += params.range__slidesPerGroup - loopedrange__slides % params.range__slidesPerGroup;
  }
  swiper.loopedrange__slides = loopedrange__slides;
  const prependrange__slidesIndexes = [];
  const appendrange__slidesIndexes = [];
  let activeIndex = swiper.activeIndex;
  if (typeof activerange__slideIndex === 'undefined') {
    activerange__slideIndex = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementIndex"])(swiper.range__slides.filter(el => el.classList.contains('swiper-range__slide-active'))[0]);
  } else {
    activeIndex = activerange__slideIndex;
  }
  const isNext = direction === 'next' || !direction;
  const isPrev = direction === 'prev' || !direction;
  let range__slidesPrepended = 0;
  let range__slidesAppended = 0;
  // prepend last range__slides before start
  if (activerange__slideIndex < loopedrange__slides) {
    range__slidesPrepended = loopedrange__slides - activerange__slideIndex;
    for (let i = 0; i < loopedrange__slides - activerange__slideIndex; i += 1) {
      const index = i - Math.floor(i / range__slides.length) * range__slides.length;
      prependrange__slidesIndexes.push(range__slides.length - index - 1);
    }
  } else if (activerange__slideIndex /* + range__slidesPerView */ > swiper.range__slides.length - loopedrange__slides * 2) {
    range__slidesAppended = activerange__slideIndex - (swiper.range__slides.length - loopedrange__slides * 2);
    for (let i = 0; i < range__slidesAppended; i += 1) {
      const index = i - Math.floor(i / range__slides.length) * range__slides.length;
      appendrange__slidesIndexes.push(index);
    }
  }
  if (isPrev) {
    prependrange__slidesIndexes.forEach(index => {
      range__slidesEl.prepend(swiper.range__slides[index]);
    });
  }
  if (isNext) {
    appendrange__slidesIndexes.forEach(index => {
      range__slidesEl.append(swiper.range__slides[index]);
    });
  }
  swiper.recalcrange__slides();
  ...
  if (range__slideTo) {
    if (prependrange__slidesIndexes.length > 0 && isPrev) {
      if (typeof range__slideRealIndex === 'undefined') {
        const currentrange__slideTranslate = swiper.range__slidesGrid[activeIndex];
        const newrange__slideTranslate = swiper.range__slidesGrid[activeIndex + range__slidesPrepended];
        const diff = newrange__slideTranslate - currentrange__slideTranslate;
        if (byMousewheel) {
          swiper.setTranslate(swiper.translate - diff);
        } else {
          swiper.range__slideTo(activeIndex + range__slidesPrepended, 0, false, true);
          if (setTranslate) {
        ...
        if (setTranslate) {
          swiper.range__slideToLoop(range__slideRealIndex, 0, false, true);
        }
      }
    } else if (appendrange__slidesIndexes.length > 0 && isNext) {
      if (typeof range__slideRealIndex === 'undefined') {
        const currentrange__slideTranslate = swiper.range__slidesGrid[activeIndex];
        const newrange__slideTranslate = swiper.range__slidesGrid[activeIndex - range__slidesAppended];
        const diff = newrange__slideTranslate - currentrange__slideTranslate;
        if (byMousewheel) {
          swiper.setTranslate(swiper.translate - diff);
        } else {
          swiper.range__slideTo(activeIndex - range__slidesAppended, 0, false, true);
          if (setTranslate) {
            swiper.touches[swiper.isHorizontal() ? 'startX' : 'startY'] += diff;
          }
        }
...
/***/ "../node_modules/swiper/core/range__slide/index.js":
/*!**************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/index.js ***!
  \**************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _range__slideTo_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./range__slideTo.js */ "../node_modules/swiper/core/range__slide/range__slideTo.js");
/* harmony import */ var _range__slideToLoop_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./range__slideToLoop.js */ "../node_modules/swiper/core/range__slide/range__slideToLoop.js");
/* harmony import */ var _range__slideNext_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./range__slideNext.js */ "../node_modules/swiper/core/range__slide/range__slideNext.js");
/* harmony import */ var _range__slidePrev_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./range__slidePrev.js */ "../node_modules/swiper/core/range__slide/range__slidePrev.js");
/* harmony import */ var _range__slideReset_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./range__slideReset.js */ "../node_modules/swiper/core/range__slide/range__slideReset.js");
/* harmony import */ var _range__slideToClosest_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./range__slideToClosest.js */ "../node_modules/swiper/core/range__slide/range__slideToClosest.js");
/* harmony import */ var _range__slideToClickedrange__slide_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./range__slideToClickedrange__slide.js */ "../node_modules/swiper/core/range__slide/range__slideToClickedrange__slide.js");
...
/* harmony default export */ __webpack_exports__["default"] = ({
  range__slideTo: _range__slideTo_js__WEBPACK_IMPORTED_MODULE_0__["default"],
  range__slideToLoop: _range__slideToLoop_js__WEBPACK_IMPORTED_MODULE_1__["default"],
  range__slideNext: _range__slideNext_js__WEBPACK_IMPORTED_MODULE_2__["default"],
  range__slidePrev: _range__slidePrev_js__WEBPACK_IMPORTED_MODULE_3__["default"],
  range__slideReset: _range__slideReset_js__WEBPACK_IMPORTED_MODULE_4__["default"],
  range__slideToClosest: _range__slideToClosest_js__WEBPACK_IMPORTED_MODULE_5__["default"],
  range__slideToClickedrange__slide: _range__slideToClickedrange__slide_js__WEBPACK_IMPORTED_MODULE_6__["default"]
});

/***/ }),

/***/ "../node_modules/swiper/core/range__slide/range__slideNext.js":
/*!******************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slideNext.js ***!
  \******************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slideNext; });
...
/***/ "../node_modules/swiper/core/range__slide/range__slidePrev.js":
/*!******************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slidePrev.js ***!
  \******************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slidePrev; });
/* eslint no-unused-vars: "off" */
function range__slidePrev(speed = this.params.speed, runCallbacks = true, internal) {
  ...
  if (params.rewind && swiper.isBeginning) {
    const lastIndex = swiper.params.virtual && swiper.params.virtual.enabled && swiper.virtual ? swiper.virtual.range__slides.length - 1 : swiper.range__slides.length - 1;
    return swiper.range__slideTo(lastIndex, speed, runCallbacks, internal);
  }
...
/***/ "../node_modules/swiper/core/range__slide/range__slideReset.js":
/*!*******************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slideReset.js ***!
  \*******************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slideReset; });
/* eslint no-unused-vars: "off" */
...
/***/ "../node_modules/swiper/core/range__slide/range__slideTo.js":
/*!****************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slideTo.js ***!
  \****************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slideTo; });
/* harmony import */ var _shared_utils_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../shared/utils.js */ "../node_modules/swiper/shared/utils.js");
...
function range__slideTo(index = 0, speed = this.params.speed, runCallbacks = true, internal, initial) {
  ...
  if (swiper.initialized && range__slideIndex !== activeIndex) {
    ...
    if (!swiper.allowrange__slidePrev && translate > swiper.translate && translate > swiper.maxTranslate()) {
      if ((activeIndex || 0) !== range__slideIndex) {
        return false;
      }
    }
  }
  if (range__slideIndex !== (previousIndex || 0) && runCallbacks) {
  ...
  let direction;
  if (range__slideIndex > activeIndex) direction = 'next';else if (range__slideIndex < activeIndex) direction = 'prev';else direction = 'reset';
  ...
  if (params.cssMode) {
    ...
    if (speed === 0) {
      ...
      if (isVirtual && !swiper._cssModeVirtualInitialSet && swiper.params.initialrange__slide > 0) {
  ...
  if (speed === 0) {
    ...
    if (!swiper.onrange__slideToWrapperTransitionEnd) {
      swiper.onrange__slideToWrapperTransitionEnd = function transitionEnd(e) {
        ...
        swiper.wrapperEl.removeEventListener('transitionend', swiper.onrange__slideToWrapperTransitionEnd);
        swiper.onrange__slideToWrapperTransitionEnd = null;
        delete swiper.onrange__slideToWrapperTransitionEnd;
        swiper.transitionEnd(runCallbacks, direction);
      };
    }
    swiper.wrapperEl.addEventListener('transitionend', swiper.onrange__slideToWrapperTransitionEnd);
  }
  return true;
}
...
/***/ "../node_modules/swiper/core/range__slide/range__slideToClickedrange__slide.js":
/*!****************************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slideToClickedrange__slide.js ***!
  \****************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slideToClickedrange__slide; });
/* harmony import */ var _shared_utils_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../shared/utils.js */ "../node_modules/swiper/shared/utils.js");
...
function range__slideToClickedrange__slide() {
  ...
  let realIndex;
  const range__slideSelector = swiper.isElement ? `swiper-range__slide` : `.${params.range__slideClass}`;
  if (params.loop) {
    if (swiper.animating) return;
    realIndex = parseInt(swiper.clickedrange__slide.getAttribute('data-swiper-range__slide-index'), 10);
    if (params.centeredrange__slides) {
      if (range__slideToIndex < swiper.loopedrange__slides - range__slidesPerView / 2 || range__slideToIndex > swiper.range__slides.length - swiper.loopedrange__slides + range__slidesPerView / 2) {
        swiper.loopFix();
        range__slideToIndex = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementIndex"])(Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementChildren"])(range__slidesEl, `${range__slideSelector}[data-swiper-range__slide-index="${realIndex}"]`)[0]);
        Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["nextTick"])(() => {
...
/***/ "../node_modules/swiper/core/range__slide/range__slideToClosest.js":
/*!***********************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slideToClosest.js ***!
  \***********************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slideToClosest; });
/* eslint no-unused-vars: "off" */
...
/***/ "../node_modules/swiper/core/range__slide/range__slideToLoop.js":
/*!********************************************************!*\
  !*** ../node_modules/swiper/core/range__slide/range__slideToLoop.js ***!
  \********************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return range__slideToLoop; });
/* harmony import */ var _shared_utils_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../shared/utils.js */ "../node_modules/swiper/shared/utils.js");
...
function range__slideToLoop(index = 0, speed = this.params.speed, runCallbacks = true, internal) {
  ...
  if (swiper.params.loop) {
    if (swiper.virtual && swiper.params.virtual.enabled) {
      // eslint-disable-next-line
      newIndex = newIndex + swiper.virtual.range__slidesBefore;
    } else {
      newIndex = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementIndex"])(swiper.range__slides.filter(range__slideEl => range__slideEl.getAttribute('data-swiper-range__slide-index') * 1 === newIndex)[0]);
    }
  }
...
/* harmony import */ var _updaterange__slides_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./updaterange__slides.js */ "../node_modules/swiper/core/update/updaterange__slides.js");
/* harmony import */ var _updateAutoHeight_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./updateAutoHeight.js */ "../node_modules/swiper/core/update/updateAutoHeight.js");
/* harmony import */ var _updaterange__slidesOffset_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./updaterange__slidesOffset.js */ "../node_modules/swiper/core/update/updaterange__slidesOffset.js");
/* harmony import */ var _updaterange__slidesProgress_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./updaterange__slidesProgress.js */ "../node_modules/swiper/core/update/updaterange__slidesProgress.js");
/* harmony import */ var _updateProgress_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./updateProgress.js */ "../node_modules/swiper/core/update/updateProgress.js");
/* harmony import */ var _updaterange__slidesClasses_js__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./updaterange__slidesClasses.js */ "../node_modules/swiper/core/update/updaterange__slidesClasses.js");
/* harmony import */ var _updateActiveIndex_js__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./updateActiveIndex.js */ "../node_modules/swiper/core/update/updateActiveIndex.js");
/* harmony import */ var _updateClickedrange__slide_js__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./updateClickedrange__slide.js */ "../node_modules/swiper/core/update/updateClickedrange__slide.js");
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  updaterange__slides: _updaterange__slides_js__WEBPACK_IMPORTED_MODULE_1__["default"],
  updateAutoHeight: _updateAutoHeight_js__WEBPACK_IMPORTED_MODULE_2__["default"],
  updaterange__slidesOffset: _updaterange__slidesOffset_js__WEBPACK_IMPORTED_MODULE_3__["default"],
  updaterange__slidesProgress: _updaterange__slidesProgress_js__WEBPACK_IMPORTED_MODULE_4__["default"],
  updateProgress: _updateProgress_js__WEBPACK_IMPORTED_MODULE_5__["default"],
  updaterange__slidesClasses: _updaterange__slidesClasses_js__WEBPACK_IMPORTED_MODULE_6__["default"],
  updateActiveIndex: _updateActiveIndex_js__WEBPACK_IMPORTED_MODULE_7__["default"],
  updateClickedrange__slide: _updateClickedrange__slide_js__WEBPACK_IMPORTED_MODULE_8__["default"]
});
...
function updateActiveIndex(newActiveIndex) {
  ...
  const getVirtualRealIndex = aIndex => {
    let realIndex = aIndex - swiper.virtual.range__slidesBefore;
    if (realIndex < 0) {
      realIndex = swiper.virtual.range__slides.length + realIndex;
    }
    if (realIndex >= swiper.virtual.range__slides.length) {
      realIndex -= swiper.virtual.range__slides.length;
    }
    return realIndex;
  };
  ...
  } else if (swiper.range__slides[activeIndex]) {
    realIndex = parseInt(swiper.range__slides[activeIndex].getAttribute('data-swiper-range__slide-index') || activeIndex, 10);
  } else {
...
function updateAutoHeight(speed) {
  ...
  const getrange__slideByIndex = index => {
    ...
    return swiper.range__slides[index];
  };
  ...
  if (swiper.params.range__slidesPerView !== 'auto' && swiper.params.range__slidesPerView > 1) {
    if (swiper.params.centeredrange__slides) {
      (swiper.visiblerange__slides || []).forEach(range__slide => {
...
/***/ "../node_modules/swiper/core/update/updateClickedrange__slide.js":
/*!****************************************************************!*\
  !*** ../node_modules/swiper/core/update/updateClickedrange__slide.js ***!
  \****************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return updateClickedrange__slide; });
function updateClickedrange__slide(e) {
  const swiper = this;
  const params = swiper.params;
  const range__slide = e.closest(`.${params.range__slideClass}, swiper-range__slide`);
  ...
  if (range__slide && range__slideFound) {
    ...
    if (swiper.virtual && swiper.params.virtual.enabled) {
      swiper.clickedIndex = parseInt(range__slide.getAttribute('data-swiper-range__slide-index'), 10);
    } else {
...
function updateProgress(translate) {
  ...
  if (params.loop) {
    const firstrange__slideIndex = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementIndex"])(swiper.range__slides.filter(el => el.getAttribute('data-swiper-range__slide-index') === '0')[0]);
    const lastrange__slideIndex = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementIndex"])(swiper.range__slides.filter(el => el.getAttribute('data-swiper-range__slide-index') * 1 === swiper.range__slides.length - 1)[0]);
    const firstrange__slideTranslate = swiper.range__slidesGrid[firstrange__slideIndex];
    const lastrange__slideTranslate = swiper.range__slidesGrid[lastrange__slideIndex];
    const translateMax = swiper.range__slidesGrid[swiper.range__slidesGrid.length - 1];
    const translateAbs = Math.abs(translate);
    if (translateAbs >= firstrange__slideTranslate) {
      progressLoop = (translateAbs - firstrange__slideTranslate) / translateMax;
    } else {
      progressLoop = (translateAbs + translateMax - lastrange__slideTranslate) / translateMax;
    }
...
/***/ "../node_modules/swiper/core/update/updaterange__slides.js":
/*!**********************************************************!*\
  !*** ../node_modules/swiper/core/update/updaterange__slides.js ***!
  \**********************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return updaterange__slides; });
/* harmony import */ var _shared_utils_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../shared/utils.js */ "../node_modules/swiper/shared/utils.js");
...
function updaterange__slides() {
  ...
  const range__slides = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementChildren"])(range__slidesEl, `.${swiper.params.range__slideClass}, swiper-range__slide`);
  ...
  range__slides.forEach(range__slideEl => {
    if (rtl) {
      range__slideEl.style.marginLeft = '';
    } else {
      range__slideEl.style.marginRight = '';
    }
    range__slideEl.style.marginBottom = '';
    range__slideEl.style.marginTop = '';
  });
  ...
  for (let i = 0; i < range__slidesLength; i += 1) {
    range__slideSize = 0;
    let range__slide;
    if (range__slides[i]) range__slide = range__slides[i];
    if (gridEnabled) {
      swiper.grid.updaterange__slide(i, range__slide, range__slidesLength, getDirectionLabel);
    }
    if (range__slides[i] && Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementStyle"])(range__slide, 'display') === 'none') continue; // eslint-disable-line

    if (params.range__slidesPerView === 'auto') {
      if (shouldResetrange__slideSize) {
        range__slides[i].style[getDirectionLabel('width')] = ``;
      }
      const range__slideStyles = getComputedStyle(range__slide);
      const currentTransform = range__slide.style.transform;
      const currentWebKitTransform = range__slide.style.webkitTransform;
      if (currentTransform) {
        range__slide.style.transform = 'none';
      }
      if (currentWebKitTransform) {
        range__slide.style.webkitTransform = 'none';
      }
      if (params.roundLengths) {
        range__slideSize = swiper.isHorizontal() ? Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementOuterSize"])(range__slide, 'width', true) : Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementOuterSize"])(range__slide, 'height', true);
      } else {
        // eslint-disable-next-line
      ...
      if (currentTransform) {
        range__slide.style.transform = currentTransform;
      }
      if (currentWebKitTransform) {
        range__slide.style.webkitTransform = currentWebKitTransform;
      }
  ...
  if (isVirtual && params.loop) {
    const size = range__slidesSizesGrid[0] + spaceBetween;
    if (params.range__slidesPerGroup > 1) {
      const groups = Math.ceil((swiper.virtual.range__slidesBefore + swiper.virtual.range__slidesAfter) / params.range__slidesPerGroup);
      const groupSize = size * params.range__slidesPerGroup;
      for (let i = 0; i < groups; i += 1) {
        snapGrid.push(snapGrid[snapGrid.length - 1] + groupSize);
      }
    }
    for (let i = 0; i < swiper.virtual.range__slidesBefore + swiper.virtual.range__slidesAfter; i += 1) {
      if (params.range__slidesPerGroup === 1) {
        snapGrid.push(snapGrid[snapGrid.length - 1] + size);
      }
      range__slidesGrid.push(range__slidesGrid[range__slidesGrid.length - 1] + size);
  ...
  if (params.spaceBetween !== 0) {
    ...
    range__slides.filter((_, range__slideIndex) => {
      ...
      return true;
    }).forEach(range__slideEl => {
      range__slideEl.style[key] = `${spaceBetween}px`;
    });
  }
  ...
  if (params.centeredrange__slides && params.cssMode && !params.centeredrange__slidesBounds) {
    ...
    Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["setCSSProperty"])(wrapperEl, '--swiper-centered-offset-after', `${swiper.size / 2 - range__slidesSizesGrid[range__slidesSizesGrid.length - 1] / 2}px`);
  ...
  if (!isVirtual && !params.cssMode && (params.effect === 'range__slide' || params.effect === 'fade')) {
    const backFaceHiddenClass = `${params.containerModifierClass}backface-hidden`;
    const hasClassBackfaceClassAdded = swiper.el.classList.contains(backFaceHiddenClass);
    if (range__slidesLength <= params.maxBackfaceHiddenrange__slides) {
...
/***/ "../node_modules/swiper/core/update/updaterange__slidesClasses.js":
/*!*****************************************************************!*\
  !*** ../node_modules/swiper/core/update/updaterange__slidesClasses.js ***!
  \*****************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return updaterange__slidesClasses; });
/* harmony import */ var _shared_utils_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../../shared/utils.js */ "../node_modules/swiper/shared/utils.js");
...
function updaterange__slidesClasses() {
  ...
  const isVirtual = swiper.virtual && params.virtual.enabled;
  const getFilteredrange__slide = selector => {
    return Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementChildren"])(range__slidesEl, `.${params.range__slideClass}${selector}, swiper-range__slide${selector}`)[0];
  };
  range__slides.forEach(range__slideEl => {
    range__slideEl.classList.remove(params.range__slideActiveClass, params.range__slideNextClass, params.range__slidePrevClass);
  });
  let activerange__slide;
  if (isVirtual) {
    if (params.loop) {
      let range__slideIndex = activeIndex - swiper.virtual.range__slidesBefore;
      if (range__slideIndex < 0) range__slideIndex = swiper.virtual.range__slides.length + range__slideIndex;
      if (range__slideIndex >= swiper.virtual.range__slides.length) range__slideIndex -= swiper.virtual.range__slides.length;
      activerange__slide = getFilteredrange__slide(`[data-swiper-range__slide-index="${range__slideIndex}"]`);
    } else {
      activerange__slide = getFilteredrange__slide(`[data-swiper-range__slide-index="${activeIndex}"]`);
    }
  } else {
    activerange__slide = range__slides[activeIndex];
  }
  if (activerange__slide) {
    // Active classes
    activerange__slide.classList.add(params.range__slideActiveClass);

    // Next range__slide
    let nextrange__slide = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementNextAll"])(activerange__slide, `.${params.range__slideClass}, swiper-range__slide`)[0];
    if (params.loop && !nextrange__slide) {
      nextrange__slide = range__slides[0];
    }
    if (nextrange__slide) {
      nextrange__slide.classList.add(params.range__slideNextClass);
    }
    // Prev range__slide
    let prevrange__slide = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_0__["elementPrevAll"])(activerange__slide, `.${params.range__slideClass}, swiper-range__slide`)[0];
    if (params.loop && !prevrange__slide === 0) {
      prevrange__slide = range__slides[range__slides.length - 1];
    }
    if (prevrange__slide) {
      prevrange__slide.classList.add(params.range__slidePrevClass);
    }
  }
...
/***/ "../node_modules/swiper/core/update/updaterange__slidesOffset.js":
/*!****************************************************************!*\
  !*** ../node_modules/swiper/core/update/updaterange__slidesOffset.js ***!
  \****************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return updaterange__slidesOffset; });
function updaterange__slidesOffset() {
  ...
  for (let i = 0; i < range__slides.length; i += 1) {
    range__slides[i].swiperrange__slideOffset = (swiper.isHorizontal() ? range__slides[i].offsetLeft : range__slides[i].offsetTop) - minusOffset;
  }
}

/***/ }),

/***/ "../node_modules/swiper/core/update/updaterange__slidesProgress.js":
/*!******************************************************************!*\
  !*** ../node_modules/swiper/core/update/updaterange__slidesProgress.js ***!
  \******************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return updaterange__slidesProgress; });
function updaterange__slidesProgress(translate = this && this.translate || 0) {
  ...
  // Visible range__slides
  range__slides.forEach(range__slideEl => {
    range__slideEl.classList.remove(params.range__slideVisibleClass);
  });
  ...
  for (let i = 0; i < range__slides.length; i += 1) {
    ...
    if (isVisible) {
      ...
      range__slides[i].classList.add(params.range__slideVisibleClass);
    }
...
function A11y({
  ...
  const handleFocus = e => {
    ...
    const range__slideEl = e.target.closest(`.${swiper.params.range__slideClass}, swiper-range__slide`);
    if (!range__slideEl || !swiper.range__slides.includes(range__slideEl)) return;
    const isActive = swiper.range__slides.indexOf(range__slideEl) === swiper.activeIndex;
    const isVisible = swiper.params.watchrange__slidesProgress && swiper.visiblerange__slides && swiper.visiblerange__slides.includes(range__slideEl);
    ...
    swiper.range__slideTo(swiper.range__slides.indexOf(range__slideEl), 0);
  };
  const initrange__slides = () => {
    const params = swiper.params.a11y;
    if (params.itemRoleDescriptionMessage) {
      addElRoleDescription(swiper.range__slides, params.itemRoleDescriptionMessage);
    }
    if (params.range__slideRole) {
      addElRole(swiper.range__slides, params.range__slideRole);
    }
    const range__slidesLength = swiper.range__slides.length;
    if (params.range__slideLabelMessage) {
      swiper.range__slides.forEach((range__slideEl, index) => {
        const range__slideIndex = swiper.params.loop ? parseInt(range__slideEl.getAttribute('data-swiper-range__slide-index'), 10) : index;
  ...
  const init = () => {
    ...
    if (nextEl) {
      nextEl.forEach(el => initNavEl(el, wrapperId, params.nextrange__slideMessage));
    }
    if (prevEl) {
      prevEl.forEach(el => initNavEl(el, wrapperId, params.prevrange__slideMessage));
    }
  ...
  on('range__slidesLengthChange snapGridLengthChange range__slidesGridLengthChange', () => {
    if (!swiper.params.a11y.enabled) return;
    initrange__slides();
  });
...
function Autoplay({
  ...
  const getrange__slideDelay = () => {
    let activerange__slideEl;
    if (swiper.virtual && swiper.params.virtual.enabled) {
      activerange__slideEl = swiper.range__slides.filter(range__slideEl => range__slideEl.classList.contains('swiper-range__slide-active'))[0];
    } else {
      activerange__slideEl = swiper.range__slides[swiper.activeIndex];
    }
    if (!activerange__slideEl) return undefined;
    const currentrange__slideDelay = parseInt(activerange__slideEl.getAttribute('data-swiper-autoplay'), 10);
    return currentrange__slideDelay;
  };
  const run = delayForce => {
    ...
    const currentrange__slideDelay = getrange__slideDelay();
    if (!Number.isNaN(currentrange__slideDelay) && currentrange__slideDelay > 0 && typeof delayForce === 'undefined') {
      delay = currentrange__slideDelay;
      autoplayDelayTotal = currentrange__slideDelay;
      autoplayDelayCurrent = currentrange__slideDelay;
    }
    ...
    const proceed = () => {
      ...
      if (swiper.params.autoplay.reverseDirection) {
        if (!swiper.isBeginning || swiper.params.loop || swiper.params.rewind) {
          swiper.range__slidePrev(speed, true, true);
          emit('autoplay');
        } else if (!swiper.params.autoplay.stopOnLastrange__slide) {
          swiper.range__slideTo(swiper.range__slides.length - 1, speed, true, true);
          emit('autoplay');
        }
      } else {
        if (!swiper.isEnd || swiper.params.loop || swiper.params.rewind) {
          swiper.range__slideNext(speed, true, true);
          emit('autoplay');
        } else if (!swiper.params.autoplay.stopOnLastrange__slide) {
          swiper.range__slideTo(0, speed, true, true);
          emit('autoplay');
        }
      }
      if (swiper.params.cssMode) {
  ...
  const pause = (internal, reset) => {
    ...
    if (reset) {
      if (range__slideChanged) {
        autoplayTimeLeft = swiper.params.autoplay.delay;
      }
      range__slideChanged = false;
      proceed();
      return;
    }
...
function EffectCards({
  ...
  extendParams({
    cardsEffect: {
      ...
      rotate: true,
      perrange__slideRotate: 2,
      perrange__slideOffset: 8
    }
  });
  const setTranslate = () => {
    const {
    ...
    for (let i = 0; i < range__slides.length; i += 1) {
      const range__slideEl = range__slides[i];
      const range__slideProgress = range__slideEl.progress;
      const progress = Math.min(Math.max(range__slideProgress, -4), 4);
      let offset = range__slideEl.swiperrange__slideOffset;
      ...
      let scale = 1;
      let rotate = -params.perrange__slideRotate * progress;
      let tXAdd = params.perrange__slideOffset - Math.abs(progress) * 0.75;
      const range__slideIndex = swiper.virtual && swiper.params.virtual.enabled ? swiper.virtual.from + i : i;
      const isSwipeToNext = (range__slideIndex === activeIndex || range__slideIndex === activeIndex - 1) && progress > 0 && progress < 1 && (isTouched || swiper.params.cssMode) && currentTranslate < startTranslate;
      const isSwipeToPrev = (range__slideIndex === activeIndex || range__slideIndex === activeIndex + 1) && progress < 0 && progress > -1 && (isTouched || swiper.params.cssMode) && currentTranslate > startTranslate;
      ...
      if (params.range__slideShadows) {
        // Set shadows
        let shadowEl = range__slideEl.querySelector('.swiper-range__slide-shadow');
        if (!shadowEl) {
          shadowEl = Object(_shared_create_shadow_js__WEBPACK_IMPORTED_MODULE_0__["default"])(params, range__slideEl);
        }
        if (shadowEl) shadowEl.style.opacity = Math.min(Math.max((Math.abs(progress) - 0.5) / 0.5, 0), 1);
      }
      range__slideEl.style.zIndex = -Math.abs(Math.round(range__slideProgress)) + range__slides.length;
      const targetEl = Object(_shared_effect_target_js__WEBPACK_IMPORTED_MODULE_2__["default"])(params, range__slideEl);
      targetEl.style.transform = transform;
    }
  };
  const setTransition = duration => {
    const transformElements = swiper.range__slides.map(range__slideEl => Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_4__["getrange__slideTransformEl"])(range__slideEl));
    transformElements.forEach(el => {
      el.style.transitionDuration = `${duration}ms`;
      el.querySelectorAll('.swiper-range__slide-shadow').forEach(shadowEl => {
...
function EffectCoverflow({
  ...
  const setTranslate = () => {
    ...
    // Each range__slide offset from center
    for (let i = 0, length = range__slides.length; i < length; i += 1) {
      const range__slideEl = range__slides[i];
      const range__slideSize = range__slidesSizesGrid[i];
      const range__slideOffset = range__slideEl.swiperrange__slideOffset;
      const centerOffset = (center - range__slideOffset - range__slideSize / 2) / range__slideSize;
      ...
      // Allow percentage to make a relative stretch for responsive range__sliders
      ...
      const targetEl = Object(_shared_effect_target_js__WEBPACK_IMPORTED_MODULE_2__["default"])(params, range__slideEl);
      targetEl.style.transform = range__slideTransform;
      range__slideEl.style.zIndex = -Math.abs(Math.round(offsetMultiplier)) + 1;
      if (params.range__slideShadows) {
        // Set shadows
        let shadowBeforeEl = isHorizontal ? range__slideEl.querySelector('.swiper-range__slide-shadow-left') : range__slideEl.querySelector('.swiper-range__slide-shadow-top');
        let shadowAfterEl = isHorizontal ? range__slideEl.querySelector('.swiper-range__slide-shadow-right') : range__slideEl.querySelector('.swiper-range__slide-shadow-bottom');
        if (!shadowBeforeEl) {
          shadowBeforeEl = Object(_shared_create_shadow_js__WEBPACK_IMPORTED_MODULE_0__["default"])(params, range__slideEl, isHorizontal ? 'left' : 'top');
        }
        if (!shadowAfterEl) {
          shadowAfterEl = Object(_shared_create_shadow_js__WEBPACK_IMPORTED_MODULE_0__["default"])(params, range__slideEl, isHorizontal ? 'right' : 'bottom');
        }
  ...
  const setTransition = duration => {
    const transformElements = swiper.range__slides.map(range__slideEl => Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_3__["getrange__slideTransformEl"])(range__slideEl));
    transformElements.forEach(el => {
      el.style.transitionDuration = `${duration}ms`;
      el.querySelectorAll('.swiper-range__slide-shadow-top, .swiper-range__slide-shadow-right, .swiper-range__slide-shadow-bottom, .swiper-range__slide-shadow-left').forEach(shadowEl => {
        shadowEl.style.transitionDuration = `${duration}ms`;
      });
    });
  };
...
function EffectCreative({
  ...
  const setTranslate = () => {
    ...
    for (let i = 0; i < range__slides.length; i += 1) {
      const range__slideEl = range__slides[i];
      const range__slideProgress = range__slideEl.progress;
      const progress = Math.min(Math.max(range__slideEl.progress, -params.limitProgress), params.limitProgress);
      let originalProgress = progress;
      if (!isCenteredrange__slides) {
        originalProgress = Math.min(Math.max(range__slideEl.originalProgress, -params.limitProgress), params.limitProgress);
      }
      const offset = range__slideEl.swiperrange__slideOffset;
...
function EffectCube({
  ...
  const createrange__slideShadows = (range__slideEl, progress, isHorizontal) => {
    let shadowBefore = isHorizontal ? range__slideEl.querySelector('.swiper-range__slide-shadow-left') : range__slideEl.querySelector('.swiper-range__slide-shadow-top');
    let shadowAfter = isHorizontal ? range__slideEl.querySelector('.swiper-range__slide-shadow-right') : range__slideEl.querySelector('.swiper-range__slide-shadow-bottom');
    if (!shadowBefore) {
      shadowBefore = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["createElement"])('div', `swiper-range__slide-shadow-${isHorizontal ? 'left' : 'top'}`);
      range__slideEl.append(shadowBefore);
    }
    if (!shadowAfter) {
      shadowAfter = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["createElement"])('div', `swiper-range__slide-shadow-${isHorizontal ? 'right' : 'bottom'}`);
  ...
  const recreateShadows = () => {
    ...
    const isHorizontal = swiper.isHorizontal();
    swiper.range__slides.forEach(range__slideEl => {
      const progress = Math.max(Math.min(range__slideEl.progress, 1), -1);
      createrange__slideShadows(range__slideEl, progress, isHorizontal);
    });
  };
  const setTranslate = () => {
    const {
    ...
    if (params.shadow) {
      if (isHorizontal) {
        cubeShadowEl = swiper.range__slidesEl.querySelector('.swiper-cube-shadow');
        if (!cubeShadowEl) {
          cubeShadowEl = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["createElement"])('div', 'swiper-cube-shadow');
          swiper.range__slidesEl.append(cubeShadowEl);
        }
    ...
    for (let i = 0; i < range__slides.length; i += 1) {
      ...
      if (isVirtual) {
        range__slideIndex = parseInt(range__slideEl.getAttribute('data-swiper-range__slide-index'), 10);
      }
      ...
      range__slideEl.style.transform = transform;
      if (params.range__slideShadows) {
        createrange__slideShadows(range__slideEl, progress, isHorizontal);
      }
    }
  ...
  const setTransition = duration => {
    ...
    range__slides.forEach(range__slideEl => {
      range__slideEl.style.transitionDuration = `${duration}ms`;
      range__slideEl.querySelectorAll('.swiper-range__slide-shadow-top, .swiper-range__slide-shadow-right, .swiper-range__slide-shadow-bottom, .swiper-range__slide-shadow-left').forEach(subEl => {
...
function EffectFade({
  ...
  const setTranslate = () => {
    ...
    for (let i = 0; i < range__slides.length; i += 1) {
      const range__slideEl = swiper.range__slides[i];
      const offset = range__slideEl.swiperrange__slideOffset;
      let tx = -offset;
      ...
      const range__slideOpacity = swiper.params.fadeEffect.crossFade ? Math.max(1 - Math.abs(range__slideEl.progress), 0) : 1 + Math.min(Math.max(range__slideEl.progress, -1), 0);
      const targetEl = Object(_shared_effect_target_js__WEBPACK_IMPORTED_MODULE_1__["default"])(params, range__slideEl);
      targetEl.style.opacity = range__slideOpacity;
...
function EffectFlip({
  ...
  const createrange__slideShadows = (range__slideEl, progress, params) => {
    let shadowBefore = swiper.isHorizontal() ? range__slideEl.querySelector('.swiper-range__slide-shadow-left') : range__slideEl.querySelector('.swiper-range__slide-shadow-top');
    let shadowAfter = swiper.isHorizontal() ? range__slideEl.querySelector('.swiper-range__slide-shadow-right') : range__slideEl.querySelector('.swiper-range__slide-shadow-bottom');
    if (!shadowBefore) {
      shadowBefore = Object(_shared_create_shadow_js__WEBPACK_IMPORTED_MODULE_0__["default"])(params, range__slideEl, swiper.isHorizontal() ? 'left' : 'top');
    }
    if (!shadowAfter) {
      shadowAfter = Object(_shared_create_shadow_js__WEBPACK_IMPORTED_MODULE_0__["default"])(params, range__slideEl, swiper.isHorizontal() ? 'right' : 'bottom');
    }
    if (shadowBefore) shadowBefore.style.opacity = Math.max(-progress, 0);
    if (shadowAfter) shadowAfter.style.opacity = Math.max(progress, 0);
  };
  const recreateShadows = () => {
    ...
    swiper.range__slides.forEach(range__slideEl => {
      let progress = range__slideEl.progress;
      if (swiper.params.flipEffect.limitRotation) {
        progress = Math.max(Math.min(range__slideEl.progress, 1), -1);
      }
      createrange__slideShadows(range__slideEl, progress, params);
    });
  };
  const setTranslate = () => {
    const {
    ...
    for (let i = 0; i < range__slides.length; i += 1) {
      ...
      range__slideEl.style.zIndex = -Math.abs(Math.round(progress)) + range__slides.length;
      if (params.range__slideShadows) {
        createrange__slideShadows(range__slideEl, progress, params);
      }
...
function Grid({
  ...
  const updaterange__slide = (i, range__slide, range__slidesLength, getDirectionLabel) => {
    ...
    if (fill === 'row' && range__slidesPerGroup > 1) {
      ...
      range__slide.style.order = newrange__slideOrderIndex;
    } else if (fill === 'column') {
    ...
    range__slide.style[getDirectionLabel('margin-top')] = row !== 0 ? spaceBetween && `${spaceBetween}px` : '';
  };
...
function HashNavigation({
  ...
  const onHashChange = () => {
    ...
    const activerange__slideHash = swiper.range__slides[swiper.activeIndex].getAttribute('data-hash');
    if (newHash !== activerange__slideHash) {
      const newIndex = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementIndex"])(Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementChildren"])(swiper.range__slidesEl, `.${swiper.params.range__slideClass}[data-hash="${newHash}"], swiper-range__slide[data-hash="${newHash}"]`)[0]);
  ...
  const setHash = () => {
    ...
    if (swiper.params.hashNavigation.replaceState && window.history && window.history.replaceState) {
      window.history.replaceState(null, null, `#${swiper.range__slides[swiper.activeIndex].getAttribute('data-hash')}` || '');
      emit('hashSet');
    } else {
      const range__slide = swiper.range__slides[swiper.activeIndex];
      const hash = range__slide.getAttribute('data-hash') || range__slide.getAttribute('data-history');
  ...
  const init = () => {
    ...
    if (hash) {
      ...
      for (let i = 0, length = swiper.range__slides.length; i < length; i += 1) {
        const range__slide = swiper.range__slides[i];
        const range__slideHash = range__slide.getAttribute('data-hash') || range__slide.getAttribute('data-history');
        if (range__slideHash === hash) {
          const index = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementIndex"])(range__slide);
...
function History({
  ...
  const setHistory = (key, index) => {
    ...
    const range__slide = swiper.range__slides[index];
    let value = slugify(range__slide.getAttribute('data-history'));
  ...
  const scrollTorange__slide = (speed, value, runCallbacks) => {
    if (value) {
      for (let i = 0, length = swiper.range__slides.length; i < length; i += 1) {
        const range__slide = swiper.range__slides[i];
        const range__slideHistory = slugify(range__slide.getAttribute('data-history'));
        if (range__slideHistory === value) {
          const index = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementIndex"])(range__slide);
  ...
  const setHistoryPopState = () => {
    ...
    scrollTorange__slide(swiper.params.speed, paths.value, false);
  };
  const init = () => {
    const window = Object(ssr_window__WEBPACK_IMPORTED_MODULE_0__["getWindow"])();
...
function Keyboard({
  ...
  function handle(event) {
    ...
    if (swiper.params.keyboard.onlyInViewport && (isPageUp || isPageDown || isArrowLeft || isArrowRight || isArrowUp || isArrowDown)) {
      ...
      if (Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementParents"])(swiper.el, `.${swiper.params.range__slideClass}, swiper-range__slide`).length > 0 && Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementParents"])(swiper.el, `.${swiper.params.range__slideActiveClass}`).length === 0) {
        return undefined;
      }
      const el = swiper.el;
...
/* harmony import */ var _methods_appendrange__slide_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./methods/appendrange__slide.js */ "../node_modules/swiper/modules/manipulation/methods/appendrange__slide.js");
/* harmony import */ var _methods_prependrange__slide_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./methods/prependrange__slide.js */ "../node_modules/swiper/modules/manipulation/methods/prependrange__slide.js");
/* harmony import */ var _methods_addrange__slide_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./methods/addrange__slide.js */ "../node_modules/swiper/modules/manipulation/methods/addrange__slide.js");
/* harmony import */ var _methods_removerange__slide_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./methods/removerange__slide.js */ "../node_modules/swiper/modules/manipulation/methods/removerange__slide.js");
/* harmony import */ var _methods_removeAllrange__slides_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./methods/removeAllrange__slides.js */ "../node_modules/swiper/modules/manipulation/methods/removeAllrange__slides.js");
...
function Manipulation({
  ...
  Object.assign(swiper, {
    appendrange__slide: _methods_appendrange__slide_js__WEBPACK_IMPORTED_MODULE_0__["default"].bind(swiper),
    prependrange__slide: _methods_prependrange__slide_js__WEBPACK_IMPORTED_MODULE_1__["default"].bind(swiper),
    addrange__slide: _methods_addrange__slide_js__WEBPACK_IMPORTED_MODULE_2__["default"].bind(swiper),
    removerange__slide: _methods_removerange__slide_js__WEBPACK_IMPORTED_MODULE_3__["default"].bind(swiper),
    removeAllrange__slides: _methods_removeAllrange__slides_js__WEBPACK_IMPORTED_MODULE_4__["default"].bind(swiper)
  });
}

/***/ }),

/***/ "../node_modules/swiper/modules/manipulation/methods/addrange__slide.js":
/*!***********************************************************************!*\
  !*** ../node_modules/swiper/modules/manipulation/methods/addrange__slide.js ***!
  \***********************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return addrange__slide; });
function addrange__slide(index, range__slides) {
  ...
  for (let i = baseLength - 1; i >= index; i -= 1) {
    const currentrange__slide = swiper.range__slides[i];
...
/***/ "../node_modules/swiper/modules/manipulation/methods/appendrange__slide.js":
/*!**************************************************************************!*\
  !*** ../node_modules/swiper/modules/manipulation/methods/appendrange__slide.js ***!
  \**************************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return appendrange__slide; });
function appendrange__slide(range__slides) {
  ...
  const appendElement = range__slideEl => {
    if (typeof range__slideEl === 'string') {
      const tempDOM = document.createElement('div');
      tempDOM.innerHTML = range__slideEl;
      range__slidesEl.append(tempDOM.children[0]);
      tempDOM.innerHTML = '';
    } else {
      range__slidesEl.append(range__slideEl);
    }
  ...
  if (typeof range__slides === 'object' && 'length' in range__slides) {
    for (let i = 0; i < range__slides.length; i += 1) {
      if (range__slides[i]) appendElement(range__slides[i]);
    }
  } else {
    appendElement(range__slides);
  }
  swiper.recalcrange__slides();
  if (params.loop) {
    swiper.loopCreate();
...
/***/ "../node_modules/swiper/modules/manipulation/methods/prependrange__slide.js":
/*!***************************************************************************!*\
  !*** ../node_modules/swiper/modules/manipulation/methods/prependrange__slide.js ***!
  \***************************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return prependrange__slide; });
function prependrange__slide(range__slides) {
  ...
  const prependElement = range__slideEl => {
    if (typeof range__slideEl === 'string') {
      const tempDOM = document.createElement('div');
      tempDOM.innerHTML = range__slideEl;
      range__slidesEl.prepend(tempDOM.children[0]);
      tempDOM.innerHTML = '';
    } else {
      range__slidesEl.prepend(range__slideEl);
    }
  };
  if (typeof range__slides === 'object' && 'length' in range__slides) {
    for (let i = 0; i < range__slides.length; i += 1) {
      if (range__slides[i]) prependElement(range__slides[i]);
    }
    newActiveIndex = activeIndex + range__slides.length;
  } else {
    prependElement(range__slides);
  }
  swiper.recalcrange__slides();
  if (params.loop) {
    swiper.loopCreate();
...
/***/ "../node_modules/swiper/modules/manipulation/methods/removeAllrange__slides.js":
/*!******************************************************************************!*\
  !*** ../node_modules/swiper/modules/manipulation/methods/removeAllrange__slides.js ***!
  \******************************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return removeAllrange__slides; });
...
/***/ "../node_modules/swiper/modules/manipulation/methods/removerange__slide.js":
/*!**************************************************************************!*\
  !*** ../node_modules/swiper/modules/manipulation/methods/removerange__slide.js ***!
  \**************************************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return removerange__slide; });
function removerange__slide(range__slidesIndexes) {
  ...
  if (typeof range__slidesIndexes === 'object' && 'length' in range__slidesIndexes) {
    for (let i = 0; i < range__slidesIndexes.length; i += 1) {
      ...
      if (swiper.range__slides[indexToRemove]) swiper.range__slides[indexToRemove].remove();
...
function Pagination({
  ...
  function onBulletClick(e) {
    ...
    e.preventDefault();
    const index = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_2__["elementIndex"])(bulletEl) * swiper.params.range__slidesPerGroup;
    if (swiper.params.loop) {
      swiper.range__slideToLoop(index);
    } else {
  ...
  function update() {
    ...
    const total = swiper.params.loop ? Math.ceil(range__slidesLength / swiper.params.range__slidesPerGroup) : swiper.snapGrid.length;
    if (swiper.params.loop) {
      current = swiper.params.range__slidesPerGroup > 1 ? Math.floor(swiper.realIndex / swiper.params.range__slidesPerGroup) : swiper.realIndex;
  ...
  function render() {
    ...
    if (params.type === 'bullets') {
      let numberOfBullets = swiper.params.loop ? Math.ceil(range__slidesLength / swiper.params.range__slidesPerGroup) : swiper.snapGrid.length;
      if (swiper.params.freeMode && swiper.params.freeMode.enabled && numberOfBullets > range__slidesLength) {
...
function Parallax({
  ...
  const setTranslate = () => {
    ...
    range__slides.forEach((range__slideEl, range__slideIndex) => {
      ...
      range__slideEl.querySelectorAll('[data-swiper-parallax], [data-swiper-parallax-x], [data-swiper-parallax-y], [data-swiper-parallax-opacity], [data-swiper-parallax-scale], [data-swiper-parallax-rotate]').forEach(subEl => {
        setTransform(subEl, range__slideProgress);
      });
    });
  };
...
function Thumb({
  ...
  function onThumbClick() {
    ...
    if (clickedrange__slide && clickedrange__slide.classList.contains(swiper.params.thumbs.range__slideThumbActiveClass)) return;
    if (typeof clickedIndex === 'undefined' || clickedIndex === null) return;
    let range__slideToIndex;
    if (thumbsSwiper.params.loop) {
      range__slideToIndex = parseInt(thumbsSwiper.clickedrange__slide.getAttribute('data-swiper-range__slide-index'), 10);
    } else {
      range__slideToIndex = clickedIndex;
    }
    if (swiper.params.loop) {
      swiper.range__slideToLoop(range__slideToIndex);
    } else {
      swiper.range__slideTo(range__slideToIndex);
    }
  }
  ...
  function update(initial) {
    ...
    thumbsSwiper.range__slides.forEach(range__slideEl => range__slideEl.classList.remove(thumbActiveClass));
    if (thumbsSwiper.params.loop || thumbsSwiper.params.virtual && thumbsSwiper.params.virtual.enabled) {
      for (let i = 0; i < thumbsToActivate; i += 1) {
        Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementChildren"])(thumbsSwiper.range__slidesEl, `[data-swiper-range__slide-index="${swiper.realIndex + i}"]`).forEach(range__slideEl => {
          range__slideEl.classList.add(thumbActiveClass);
        });
      }
    } else {
      for (let i = 0; i < thumbsToActivate; i += 1) {
        if (thumbsSwiper.range__slides[swiper.realIndex + i]) {
          thumbsSwiper.range__slides[swiper.realIndex + i].classList.add(thumbActiveClass);
        }
      }
    }
    ...
    if (swiper.realIndex !== thumbsSwiper.realIndex || useOffset) {
      ...
      let direction;
      if (thumbsSwiper.params.loop) {
        const newThumbsrange__slide = thumbsSwiper.range__slides.filter(range__slideEl => range__slideEl.getAttribute('data-swiper-range__slide-index') === `${swiper.realIndex}`)[0];
        newThumbsIndex = thumbsSwiper.range__slides.indexOf(newThumbsrange__slide);
      ...
      if (thumbsSwiper.visiblerange__slidesIndexes && thumbsSwiper.visiblerange__slidesIndexes.indexOf(newThumbsIndex) < 0) {
        if (thumbsSwiper.params.centeredrange__slides) {
          ...
          // newThumbsIndex = newThumbsIndex - range__slidesPerView + 1;
        }
...
function Virtual({
  ...
  function renderrange__slide(range__slide, index) {
    ...
    // eslint-disable-next-line
    let range__slideEl;
    if (params.renderrange__slide) {
      range__slideEl = params.renderrange__slide.call(swiper, range__slide, index);
      if (typeof range__slideEl === 'string') {
        tempDOM.innerHTML = range__slideEl;
        range__slideEl = tempDOM.children[0];
      }
    } else if (swiper.isElement) {
      range__slideEl = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["createElement"])('swiper-range__slide');
    } else {
      range__slideEl = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["createElement"])('div', swiper.params.range__slideClass);
    }
    range__slideEl.setAttribute('data-swiper-range__slide-index', index);
    if (!params.renderrange__slide) {
      range__slideEl.textContent = range__slide;
    }
  ...
  function update(force) {
    ...
    if (centeredrange__slides) {
      ...
      range__slidesBefore = (isLoop ? range__slidesPerView : range__slidesPerGroup) + addrange__slidesBefore;
    }
    let from = activeIndex - range__slidesBefore;
    let to = activeIndex + range__slidesAfter;
    if (!isLoop) {
      from = Math.max(from, 0);
      to = Math.min(to, range__slides.length - 1);
    }
    let offset = (swiper.range__slidesGrid[from] || 0) - (swiper.range__slidesGrid[0] || 0);
    if (isLoop && activeIndex >= range__slidesBefore) {
      from -= range__slidesBefore;
      if (!centeredrange__slides) offset += swiper.range__slidesGrid[0];
    } else if (isLoop && activeIndex < range__slidesBefore) {
      from = -range__slidesBefore;
      if (centeredrange__slides) offset += swiper.range__slidesGrid[0];
    }
    ...
    if (previousFrom === from && previousTo === to && !force) {
      if (swiper.range__slidesGrid !== previousrange__slidesGrid && offset !== previousOffset) {
        swiper.range__slides.forEach(range__slideEl => {
          range__slideEl.style[offsetProp] = `${offset}px`;
        });
      }
      swiper.updateProgress();
      emit('virtualUpdate');
    ...
    const appendIndexes = [];
    const getrange__slideIndex = index => {
      let range__slideIndex = index;
      if (index < 0) {
        range__slideIndex = range__slides.length + index;
      } else if (range__slideIndex >= range__slides.length) {
        // eslint-disable-next-line
        range__slideIndex = range__slideIndex - range__slides.length;
      }
      return range__slideIndex;
    };
    if (force) {
      swiper.range__slidesEl.querySelectorAll(`.${swiper.params.range__slideClass}, swiper-range__slide`).forEach(range__slideEl => {
        range__slideEl.remove();
      });
    } else {
      for (let i = previousFrom; i <= previousTo; i += 1) {
        if (i < from || i > to) {
          const range__slideIndex = getrange__slideIndex(i);
          swiper.range__slidesEl.querySelectorAll(`.${swiper.params.range__slideClass}[data-swiper-range__slide-index="${range__slideIndex}"], swiper-range__slide[data-swiper-range__slide-index="${range__slideIndex}"]`).forEach(range__slideEl => {
            range__slideEl.remove();
          });
        }
      }
    }
    const loopFrom = isLoop ? -range__slides.length : 0;
    const loopTo = isLoop ? range__slides.length * 2 : range__slides.length;
    for (let i = loopFrom; i < loopTo; i += 1) {
      if (i >= from && i <= to) {
        const range__slideIndex = getrange__slideIndex(i);
        if (typeof previousTo === 'undefined' || force) {
          appendIndexes.push(range__slideIndex);
        } else {
          if (i > previousTo) appendIndexes.push(range__slideIndex);
          if (i < previousFrom) prependIndexes.push(range__slideIndex);
        }
      }
    }
    appendIndexes.forEach(index => {
      swiper.range__slidesEl.append(renderrange__slide(range__slides[index], index));
    });
    if (isLoop) {
      for (let i = prependIndexes.length - 1; i >= 0; i -= 1) {
        const index = prependIndexes[i];
        swiper.range__slidesEl.prepend(renderrange__slide(range__slides[index], index));
      }
    } else {
    ...
    Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementChildren"])(swiper.range__slidesEl, '.swiper-range__slide, swiper-range__slide').forEach(range__slideEl => {
      range__slideEl.style[offsetProp] = `${offset}px`;
    });
    onRendered();
  }
  ...
  function prependrange__slide(range__slides) {
    ...
    if (swiper.params.virtual.cache) {
      ...
      Object.keys(cache).forEach(cachedIndex => {
        ...
        const cachedElIndex = cachedEl.getAttribute('data-swiper-range__slide-index');
        if (cachedElIndex) {
          cachedEl.setAttribute('data-swiper-range__slide-index', parseInt(cachedElIndex, 10) + numberOfNewrange__slides);
        }
  ...
  on('beforeInit', () => {
    ...
    let domrange__slidesAssigned;
    if (typeof swiper.passedParams.virtual.range__slides === 'undefined') {
      const range__slides = [...swiper.range__slidesEl.children].filter(el => el.matches(`.${swiper.params.range__slideClass}, swiper-range__slide`));
      if (range__slides && range__slides.length) {
        swiper.virtual.range__slides = [...range__slides];
        domrange__slidesAssigned = true;
        range__slides.forEach((range__slideEl, range__slideIndex) => {
          range__slideEl.setAttribute('data-swiper-range__slide-index', range__slideIndex);
          swiper.virtual.cache[range__slideIndex] = range__slideEl;
          range__slideEl.remove();
        });
      }
    }
    if (!domrange__slidesAssigned) {
...
function Zoom({
  ...
  Object.defineProperty(swiper.zoom, 'scale', {
    ...
    set(value) {
      if (scale !== value) {
        ...
        const range__slideEl = gesture.range__slideEl;
  ...
  function getrange__slideSelector() {
    return swiper.isElement ? `swiper-range__slide` : `.${swiper.params.range__slideClass}`;
  }
  function eventWithinrange__slide(e) {
    const range__slideSelector = getrange__slideSelector();
    if (e.target.matches(range__slideSelector)) return true;
    if (swiper.range__slides.filter(range__slideEl => range__slideEl.contains(e.target)).length > 0) return true;
    return false;
  }
  ...
  function onGestureStart(e) {
    if (!eventWithinrange__slide(e)) return;
    ...
    if (!gesture.range__slideEl) {
      gesture.range__slideEl = e.target.closest(`.${swiper.params.range__slideClass}, swiper-range__slide`);
      if (!gesture.range__slideEl) gesture.range__slideEl = swiper.range__slides[swiper.activeIndex];
      let imageEl = gesture.range__slideEl.querySelector(`.${params.containerClass}`);
      if (imageEl) {
  ...
  function onTouchMove(e) {
    if (!eventWithinrange__slide(e) || !eventWithinZoomContainer(e)) return;
    const zoom = swiper.zoom;
    if (!gesture.imageEl) return;
    swiper.allowClick = false;
    ...
    if (!image.isMoved) {
      ...
      gesture.range__slideWidth = gesture.range__slideEl.offsetWidth;
      gesture.range__slideHeight = gesture.range__slideEl.offsetHeight;
  ...
  function zoomIn(e) {
    ...
    if (!gesture.range__slideEl) {
      ...
      if (!gesture.range__slideEl) {
        if (swiper.params.virtual && swiper.params.virtual.enabled && swiper.virtual) {
          gesture.range__slideEl = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementChildren"])(swiper.range__slidesEl, `.${swiper.params.range__slideActiveClass}`)[0];
        } else {
          gesture.range__slideEl = swiper.range__slides[swiper.activeIndex];
        }
      }
      let imageEl = gesture.range__slideEl.querySelector(`.${params.containerClass}`);
      if (imageEl) {
    ...
    gesture.range__slideEl.classList.add(`${params.zoomedrange__slideClass}`);
    let touchX;
    let touchY;
    let offsetX;
    let offsetY;
    ...
    if (e && !(currentScale === 1 && forceZoomRatio)) {
      range__slideWidth = gesture.range__slideEl.offsetWidth;
      range__slideHeight = gesture.range__slideEl.offsetHeight;
      offsetX = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementOffset"])(gesture.range__slideEl).left + window.scrollX;
      offsetY = Object(_shared_utils_js__WEBPACK_IMPORTED_MODULE_1__["elementOffset"])(gesture.range__slideEl).top + window.scrollY;
  ...
  function zoomOut() {
    ...
    gesture.imageEl.style.transitionDuration = '300ms';
    gesture.imageEl.style.transform = 'translate3d(0,0,0) scale(1)';
    gesture.range__slideEl.classList.remove(`${params.zoomedrange__slideClass}`);
    gesture.range__slideEl = undefined;
  }
...
function createShadow(params, range__slideEl, side) {
  ...
  const shadowContainer = Object(_utils_js__WEBPACK_IMPORTED_MODULE_0__["getrange__slideTransformEl"])(range__slideEl);
  let shadowEl = shadowContainer.querySelector(`.${shadowClass}`);
  if (!shadowEl) {
    shadowEl = Object(_utils_js__WEBPACK_IMPORTED_MODULE_0__["createElement"])('div', `swiper-range__slide-shadow${side ? `-${side}` : ''}`);
...
function effectInit(params) {
  ...
  on('transitionEnd', () => {
    if (swiper.params.effect !== effect) return;
    if (recreateShadows) {
      if (!getEffectParams || !getEffectParams().range__slideShadows) return;
      // remove shadows
      swiper.range__slides.forEach(range__slideEl => {
        range__slideEl.querySelectorAll('.swiper-range__slide-shadow-top, .swiper-range__slide-shadow-right, .swiper-range__slide-shadow-bottom, .swiper-range__slide-shadow-left').forEach(shadowEl => shadowEl.remove());
      });
      // create new one
      recreateShadows();
    }
  ...
  on('virtualUpdate', () => {
    if (swiper.params.effect !== effect) return;
    if (!swiper.range__slides.length) {
      requireUpdateOnVirtual = true;
    }
    requestAnimationFrame(() => {
      if (requireUpdateOnVirtual && swiper.range__slides && swiper.range__slides.length) {
        setTranslate();
...
function effectTarget(effectParams, range__slideEl) {
  const transformEl = Object(_utils_js__WEBPACK_IMPORTED_MODULE_0__["getrange__slideTransformEl"])(range__slideEl);
  if (transformEl !== range__slideEl) {
...
function effectVirtualTransitionEnd({
  ...
  const getrange__slide = el => {
    if (!el.parentElement) {
      // assume shadow root
      const range__slide = swiper.range__slides.filter(range__slideEl => range__slideEl.shadowEl && range__slideEl.shadowEl === el.parentNode)[0];
      return range__slide;
    }
  ...
  if (swiper.params.virtualTranslate && duration !== 0) {
    ...
    if (allrange__slides) {
      ...
      transitionEndTarget = transformElements.filter(transformEl => {
        const el = transformEl.classList.contains('swiper-range__slide-transform') ? getrange__slide(transformEl) : transformEl;
...
const processLazyPreloader = (swiper, imageEl) => {
  ...
  const range__slideSelector = () => swiper.isElement ? `swiper-range__slide` : `.${swiper.params.range__slideClass}`;
  const range__slideEl = imageEl.closest(range__slideSelector());
  if (range__slideEl) {
    const lazyEl = range__slideEl.querySelector(`.${swiper.params.lazyPreloaderClass}`);
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "getrange__slideTransformEl", function() { return getrange__slideTransformEl; });
...
function getrange__slideTransformEl(range__slideEl) {
  return range__slideEl.querySelector('.swiper-range__slide-transform') || range__slideEl.shadowEl && range__slideEl.shadowEl.querySelector('.swiper-range__slide-transform') || range__slideEl;
}
